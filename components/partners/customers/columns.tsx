...
import { Pencil, Trash2, MoreHorizontal, Mail, Phone, ToggleRight, ToggleLeft } from 'lucide-react'
...
export function getCustomerColumns(
  ...
  columns.push({
    ...
    cell: ({ row }) => (
      <div>
        <span className={`font-medium ${row.original.is_active ? 'text-neutral-900' : 'text-neutral-400'}`}>
          {row.original.name}
        </span>
        {row.original.customer_code && (
          <span className="ml-2 text-xs text-neutral-500">({row.original.customer_code})</span>
        )}
  ...
  columns.push({
    ...
    cell: ({ row }) => {
      ...
      const country = row.original.billing_country
