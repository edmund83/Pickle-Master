...
test.describe('Inventory Items - Create', () => {
  ...
  test('40. Add item with minimum stock threshold', async ({ page }) => {
    ...
    // Should have helper text about alerts
    await expect(page.getByText(/alert when below/i)).toBeVisible()
  })
  ...
  test('46. Add item with tags', async ({ page }) => {
    ...
    const tagsSection = page.locator('[data-testid="tags"], [class*="tags"]').first()
  ...
  test('47. Add item with custom field values', async ({ page }) => {
    // Custom fields section should be visible (if enabled)
    const customFieldsSection = page.getByText(/custom fields/i)
    const isVisible = await customFieldsSection.isVisible().catch(() => false)
  ...
  test('50. Add item with tracking mode set to Lot', async ({ page }) => {
    // Check for tracking mode selector
    const trackingMode = page.locator('select[name="tracking_mode"], [data-testid="tracking-mode"]').first()
    const isVisible = await trackingMode.isVisible().catch(() => false)
  ...
  test('52. Add item with shipping dimensions', async ({ page }) => {
    ...
    const dimensionSection = page.locator('[data-testid="dimensions"], [class*="dimension"]').first()
    const isVisible = await dimensionSection.isVisible().catch(() => false)
