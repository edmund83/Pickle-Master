-- Fix: Set sales_orders.pick_list_id when generating a pick list from a sales order.
-- create_delivery_order_from_pick_list looks up the sales order by pick_list_id;
-- without this, "Create Delivery Order" fails with "No sales order linked to this pick list".
...
BEGIN
    ...
    -- Update sales order: link pick list and set status (required for create_delivery_order_from_pick_list)
    UPDATE sales_orders
    SET status = 'picking',
