...
- [ ] Add explicit `tenant_id` checks in all server actions before update/delete:
  - [ ] `app/actions/purchase-orders.ts` - `updatePurchaseOrder`, `deletePurchaseOrder`
  - [ ] `app/actions/pick-lists.ts` - `updatePickList`, `deletePickList`
  - [ ] `app/actions/checkouts.ts` - checkout mutations
  - [ ] `app/actions/receives.ts` - receive mutations
- [ ] Add role-based permission checks for destructive actions (delete, cancel, status changes)
- [ ] Verify all fetchers include `tenant_id` filter (not just `id`):
...
- [ ] Add Zod schemas for all server action inputs:
  - [ ] PO fields (dates, quantities, addresses)
  - [ ] Receive fields (expected/received dates, quantities)
  - [ ] Vendor creation (email, phone validation)
  - [ ] Move operations (quantity bounds)
- [ ] Add server-side length limits for free-text fields (notes, addresses)
- [ ] Clamp quantities to prevent over-receipt (received ≤ ordered)
- [ ] Validate serial counts match item quantities before completion
